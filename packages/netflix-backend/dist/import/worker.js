...
async function searchTMDB(title, year) {
    ...
    const options = {
        ...
        headers: {
            ...
            Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI4ZjkxZmJmMjVmOGY5MzM2MWU5Yjc0MDVmYTU0NDE1NyIsIm5iZiI6MTcwNDk5Mzc1My4zNzEsInN1YiI6IjY1YTAyM2Q5MjdkYjYxMDEyNzk4NjYxNSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.roO7_rw2FyxvExevz5oFaUY785nbVshk8YwRIYz3ers'
        }
    };
...
async function downloadAndConvertTMDBImage(tmdbPath, baseName, widths) {
    if (!tmdbPath)
        return {};
...
async function handleVideoFile(filepath) {
    ...
    let movieData = {
        ...
        overview: movie.overview ?? '',
        video: {
            localPath: path_1.default.resolve(filepath),
            provider: 'local'
        }
    };
    // Télécharger et convertir images TMDB en Avif
    if (movie.posterPath) {
        const posterFiles = await downloadAndConvertTMDBImage(movie.posterPath, `${movie.tmdbId}-poster`, [100, 300, 500]);
    }
    if (movie.backdropPath) {
        const backdropFiles = await downloadAndConvertTMDBImage(movie.backdropPath, `${movie.tmdbId}-background`, [300, 1000]);
    }
    // Merge avec existant si nécessaire
    if (fs_1.default.existsSync(movieJsonPath)) {
        const existing = JSON.parse(fs_1.default.readFileSync(movieJsonPath, 'utf-8'));
...
async function tmdbGetPeopleById(personId) {
...
async function tmdbGetMovieById(movieId) {
...
// Worker : traitement d'un fichier vidéo
// -----------------------------
async function handleVideoById(movieId) {
    ...
    const movieJsonPath = path_1.default.join('./data/movies', `${movieId}-detail.json`);
    let movieData = {};
    if (fs_1.default.existsSync(movieJsonPath)) {
    ...
    movieData.categories = movieTmdb.genres.map((genre) => genre.id);
...
// -----------------------------
async function handlePeopleId(peopleId) {
    const filename = path_1.default.basename(`./data/people/${peopleId}-profil.json`);
    ...
    const peopleJsonPath = path_1.default.join('./data/people', `${peopleId}-profil.json`);
    let peopleData = {};
    if (fs_1.default.existsSync(peopleJsonPath)) {
        peopleData = JSON.parse(fs_1.default.readFileSync(peopleJsonPath, 'utf-8'));
    }
    ...
    if (!peopleData.profil) {
        if (peopleTmdb.profile_path) {
            ...
            if (res.ok) {
                const buffer = Buffer.from(await res.arrayBuffer());
                const filePath = path_1.default.join(`./data/assets/people/${peopleData.id}-profil`);
    ...
    if (!(0, people_validation_1.validatePersonDetail)(peopleData)) {
        throw new Error('Invalid PersonDetail JSON');
    }
    else {
        fs_1.default.writeFileSync(peopleJsonPath, JSON.stringify(peopleData, null, 2), 'utf-8');
        console.log(`[JSON] People JSON mis à jour: ${peopleJsonPath}`);
    }
}
const people_validation_1 = require("../validation/people.validation");
...
function writeJSON(fileName, jsonData) {
    ...
    return fs_1.default.writeFileSync(fileName, JSON.stringify(jsonData), 'utf-8');
}
handleVideoById(111);
